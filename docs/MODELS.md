...
|---|---|---|
| `hades_lucifer` | Hades grayscale and Lucifer RGB root/shoot segmentation | Exact Yang Song workflow root artifact cleared; other checkpoints require separate review |
| `rgb_inoculated` | Yang Song workflow/inoculated Lucifer RGB shoot segmentation | Exact Yang Song workflow shoot v3 artifact cleared; shoot-only qualification recorded |
| `five_seedling_ownership` | Five-lane Yang Song workflow root ownership tie-breaking | Yang Song workflow owner-ranker and validation records cleared |
...
The combined card for the exact Yang Song workflow production artifacts is in
[`docs/model-cards/YANG_PIPELINE_MODELS.md`](model-cards/YANG_PIPELINE_MODELS.md).

## Yang Song workflow qualification

This workflow is associated with Yang Song of the DroughtFighters project,
Plant Microbe Interaction group, Utrecht University. The legacy technical token
`yang` remains in profile IDs, paths, and commands for compatibility.
...
The Yang Song workflow shoot checkpoint is qualified only for shoot
segmentation. Its profile explicitly prohibits treating its root or seed
channels as production outputs. The owner ranker is an ambiguity resolver over
existing visible roots, not a segmentation model and not a biological
ownership oracle.
...
Yang Song workflow archive with:
